...
HTML;
	...
	//$row = $db->super_query( "SELECT count(*) as count FROM ninacoder_users WHERE user_group='$id'" );
	
	if( ! $row['count'] ) {
		//$db->query( "DELETE FROM ninacoder_usergroups WHERE id = '$id'" );
		//@unlink( ENGINE_DIR . '/cache/system/usergroup.php' );
		//clear_cache();
		//msg( "info", $lang['all_info'], $lang['group_del'], "$PHP_SELF?mod=usergroup" );
	} else {
		if( $grouplevel and $grouplevel != $id ) {
			//$db->query( "UPDATE ninacoder_users set user_group='$grouplevel' WHERE user_group='$id'" );
			//$db->query( "DELETE FROM ninacoder_usergroups WHERE id = '$id'" );
			//@unlink( ENGINE_DIR . '/cache/system/usergroup.php' );
			//clear_cache();
			//msg( "info", $lang['all_info'], $lang['group_del'], "$PHP_SELF?mod=usergroup" );
	...
	if( $group_name == "" ) {
		...
		$id = intval( $_REQUEST['id'] );
		$db->query( "UPDATE ninacoder_usergroups set group_name='$group_name', allow_admin='$allow_admin', allow_m_artists='$allow_m_artists', 
...
HTML;
	...
	$db->query( "SELECT user_group, count(*) as count FROM ninacoder_users GROUP BY user_group" );
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Usergroup</h1>
    </div>
</div>
<div class="row">
