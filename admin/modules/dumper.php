...
<head>
    ...
    <title>NiNaCoder Dumper</title>
</head>
<body style='font-family: system,-apple-system,".SFNSText-Regular","San Francisco",Roboto,"Segoe UI","Helvetica Neue","Lucida Grande",sans-serif;''>
HTML;
/***************************************************************************\
...
if( $member_id['user_group'] != 1 ) {
    die("<strong>Error!</strong> You don't have permission in this place!");
}
...
class dumper {
    ...
    function backup() {
                        ...
                        for($k = 0; $k < $fields; $k++){
                            if ($NumericColumn[$k])
                                ...
                                if(function_exists('mysqli_connect'))
                                    $row[$k] = isset($row[$k]) ? "'" .  mysqli_real_escape_string (@mysqli_connect(MYSQLHOST, DBNUSER, MYSQLDBPASS), $row[$k]) . "'" : "NULL";
                                else $row[$k] = isset($row[$k]) ? "'" .  mysqli_escape_string ($row[$k]) . "'" : "NULL";
                            }
                        }
        ...
        echo tpl_l("The size of the database: " . round($this->size / 1024, 2) . " KB", C_RESULT);
        $filesize = round(filesize(PATH . $this->filename) / 1024, 2) . " KB";
        ...
        if(!defined('AUTOMODE'))
        {
            echo "<SCRIPT>documentation.getElementById('back').disabled = 0; with (documentation.getElementById('save')) {style.display = ''; }</SCRIPT>";
        }
...
function s(st, so){
	documentation.getElementById('st_tab').width = st ? st + '%' : '1';
	documentation.getElementById('so_tab').width = so ? so + '%' : '1';
}
function l(str, color){
	...
	with(documentation.getElementById('logarea')){
		if (!WidthLocked){
